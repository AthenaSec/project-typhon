...
Target Agent (mock vulnerable chatbot)
        ↓ attacked by
Red Team Engine (pick an engine — native / promptfoo / future — for generation+delivery; judge.py always grades)
        ↓ logs to
...
- **Attack packs and article mappings are data, not code.** They live in YAML (`redteam_engine/native/attack_packs/*.yaml`, `redteam_engine/promptfoo/promptfooconfig.yaml`, `compliance/article_map.yaml`) so they're easy to read, edit, and eventually show to non-engineers. These files are the actual differentiator of the demo — treat them as first-class, not boilerplate.
- **Attack engines generate and deliver attacks; they never grade them.** `redteam_engine/` is structured as one shared judge (`judge.py`) plus a folder per engine (`native/`, `promptfoo/`, and any future engine — PyRIT, etc.). An engine's own job stops at producing a prompt + collecting the target's response; `judge.py` is the single source of truth for pass/fail, because its rubric is written for this project's specific EU AI Act mapping, not a generic assertion. A concrete reason this matters: promptfoo's built-in grader was observed giving a false negative — "pass" with reason "Model refused the request" — on a response that had verbatim leaked the target's system prompt (see `redteam_engine/promptfoo/runner.py` docstring). Never wire an engine's own grading verdict directly into findings.
...
- Node.js 20+ is required only for the promptfoo engine (`--engine promptfoo`); it's invoked via `npx promptfoo@latest` as a subprocess, never vendored into the repo.
